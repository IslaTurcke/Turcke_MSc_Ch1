...
# clean up
rm(rvc, rvc_IDs, rvc_2, rvc_3, rvc_4, mvs, mvs_IDs, mvs_1, mvs_2, mvs_3, mvs_4)
...
rvc_start <- rvc_clip %>% 
  select(SOURCE, ID_SURV, ID_SITE, DATE, LON_DEG, LAT_DEG, LON_M, LAT_M,
         SPECIES_CODE, NUM, DENSITY, LEN)

mvs_start <- mvs_clip %>%
  select(SOURCE, ID_SURV, ID_SITE, DATE, LON_DEG, LAT_DEG, LON_M, LAT_M,
         SPECIES_CODE, NO, DENSITY, MIN_LEN, AVE_LEN, MAX_LEN)

# write out datasets as .csv files in "My_Datasets" folder in Isla_HSMs 
write_csv(rvc_start, paste0(species_wd, "RVC_Start.csv"), append = FALSE)
write_csv(mvs_start, paste0(species_wd, "MVS_Start.csv"), append = FALSE)