...
libraries("ENMTools","sf","terra","here","tidyverse","dplyr","rJava","parallel")
...
# number of cores to use for terra::predict() in the my.identity.test.onerep() function
cores = 1
...
SP1_occ <- read.csv(here("Occurrence_Data","Permutations","Perm1_Sp1.csv"), row.names = 1)
SP2_occ <- read.csv(here("Occurrence_Data","Permutations","Perm1_Sp2.csv"), row.names = 1)
...
idtest_output <- my.identity.test.onerep(species.1.perm = SP1_enm, species.2.perm = SP2_enm, 
                                  env, clamp = FALSE, temp_dir = temp_dir, cores = cores, verbose = TRUE)

# Write the raw results to a file (CLUSTER EDIT)
write.csv(idtest_output, paste0("IdentityTest_Results/BP_MP/", "April_5_overlap.csv"))
